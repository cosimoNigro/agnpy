...
class ChangCooperSolver:
    ...
    def run(self):
        ...
        for i in range(len(self.t) - 1):
            ...
            B_t = self.B_o*(self.R_o/(self.R_o+(self.t[i]*c.cgs)*np.tan(self.theta_open)))
            eta_esc = 1.5
            R_t = self.R_o + (self.t[i] * c.cgs) * np.tan(self.theta_open)
            t_esc = eta_esc * R_t / c.cgs
            coeff = 0
            escape_term = coeff * (dt / t_esc).decompose().value
            loss_term = (dt * gamma_loss(self.gamma_midpts,B_t)[1:]).decompose().value / self.delta_gamma
            V2 = 1 + loss_term + escape_term
            V3 = -(dt * gamma_loss(self.gamma_midpts,B_t)[1:]).decompose().value / self.delta_gamma
