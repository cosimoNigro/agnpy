...
class Absorption:
    ...
    def _opacity_disk(self, nu):
        """opacity generated by the SS Disk
        ...
        _s = _epsilon_1 * _epsilon * (1 + self.blob.z) / 2 * (1 - _cos_psi) 
        _integrand_mu = _phi_disk_mu / (np.power(_l, 3) * _epsilon * np.power(np.power(_mu, -2) - 1, 3 / 2)) 
