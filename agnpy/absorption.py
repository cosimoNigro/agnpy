...
class Absorption:
    ...
    def set_l_tilde(self, l_size=50):
        """Set the range of integration for the distance.
        
        Use a distance scaled with the gravitational radius: 
        if the target is not a SS disk - the only target object with the BH mass 
        among its attributes - use the gravitational radius of a 10^9 solar 
        masses BH for scaling :math:`R_g(M_{\rm BH} = 10^9\ M_{\odot})`
        
        Set also an attribute with the gravitational radius, to be used in
        further calculations"""

        if isinstance(self.target, SSDisk):
            self.R_g = self.target.R_g
        else:
            self.R_g = (G * 1e9 * M_sun / c ** 2).to("cm")

        r_tilde = (self.r / self.R_g).to_value("")
        self.l_tilde = np.logspace(0, 4, 50) * r_tilde
    ...
    def _opacity_disk(self, nu):
        ...
        for _epsilon_1 in epsilon_1:
            ...
            integral_l_tilde = np.trapz(integrand_l_tilde, self.l_tilde, axis=0)
            tau_epsilon_1 = np.append(tau_epsilon_1, integral_l_tilde)
        prefactor = (
            3
            * self.target.L_disk
            / ((4 * np.pi) ** 2 * m_e * c ** 3 * self.target.eta * self.target.R_g)
        )
