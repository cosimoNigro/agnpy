...
class TestSynchrotron:
    ...
    def test_ssa_synch_pwl_sed(self):
        """test SSA SED generated by a power law of electrons against the one 
        generated with jetset version 1.1.2"""
        sampled_ssa_sed_table = np.loadtxt(
            f"{tests_dir}/sampled_seds/synch_ssa_pwl_jetset_1.1.2.txt",
            delimiter=",",
            comments="#",
        )
        # same parameters used to produce the jetset SED
        norm = 1e2 * u.Unit("cm-3")
        spectrum_dict = {
            "type": "PowerLaw",
            "parameters": {"p": 2, "gamma_min": 2, "gamma_max": 1e6},
        }
        blob_ssa_pwl = Blob(
            R_b=5e15 * u.cm,
            z=0.1,
        ...
        make_sed_comparison_plot(
            ...
            sampled_ssa_sed,
            agnpy_ssa_sed,
            "Self Absorbed Synchrotron, power law electrons",
            "ssa_pwl_comparison_jetset_1.1.2",
        )
        plt.show()
        # requires that the SED points deviate less than 15% from the figure
        assert u.allclose(
            agnpy_ssa_sed, sampled_ssa_sed, atol=0 * u.Unit("erg cm-2 s-1"), rtol=0.10
        )
