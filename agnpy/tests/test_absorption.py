...
disk_test = SSDisk(
    1.2 * 1e9 * M_sun.cgs, 2e46 * u.Unit("erg s-1"), 1 / 12, 6, 200, R_g_units=True
)
blr_test = SphericalShellBLR(
    2e46 * u.Unit("erg s-1"), 0.024, "Lyalpha", 1e17 * u.cm
)
dt_test = RingDustTorus(2e46 * u.Unit("erg s-1"), 0.1, 1e3 * u.K)
...
class TestAbsorption:
    ...
    def test_absorption_blr_reference_tau(self):
        """test agnpy gamma-gamma optical depth for Broad Line Region against 
        ...
        # array of sampled taus
        sampled_tau = np.loadtxt(
            f"{tests_dir}/sampled_taus/tau_blr_lyman_alpha_figure_14_finke_2016.txt",
            delimiter=",",
            comments="#",
        )
        E_ref = sampled_tau[:, 0] * u.GeV
    ...
    def test_absorption_dt_reference_tau(self):
        """test agnpy gamma-gamma optical depth for Dust Torus against 
        the one in Figure 14 of Finke 2016"""
        # array of energies used in figure 14 of Finke
        r = 1.1 * 1e16 * u.cm
        E = np.logspace(0, 5) * u.GeV
        nu = E.to("Hz", equivalencies=u.spectral())
        absorption_dt = Absorption(pwl_blob_test, dt_test, r=r)
        tau = absorption_dt.tau(nu)
        # array of sampled taus
        sampled_tau = np.loadtxt(
            f"{tests_dir}/sampled_taus/tau_dt_figure_14_finke_2016.txt",
            delimiter=",",
            comments="#",
        )
        E_ref = sampled_tau[:, 0] * u.GeV
        ...
        ax.loglog(E, tau, marker=".", ls="--", label="agnpy")
        ax.legend()
        ax.set_title("Absorption Dust Torus")
        ax.set_xlabel(r"$E\,/\,{\rm GeV}$", fontsize=12)
        ax.set_ylabel(r"$\tau_{\gamma \gamma}$", fontsize=12)
        fig.savefig(
            f"{tests_dir}/crosscheck_figures/tau_dt_comaprison_figure_14_finke_2016.png"
        )
        assert True
