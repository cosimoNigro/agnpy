...
def P_sync(
    ...
    Note: Input correctly shaped arrays (B_x, gamma_e, N_e_xg, nu_obs) as per emitter geometry
    """       
    ...
    return Z_eta, P_synch.to("erg Hz-1 s-1")
...
class SynchrotronCone(RadiativeProcess):
    ...
    def flux_obs(self, nu_obs):
        ...
        self.Z_eta, L_v_fluid = P_sync(nu_fluid_reshaped, 
                     B_x_reshaped, 
                     self.emitter.x, 
                     gamma_e_reshaped, 
                     N_e_xg_reshaped,
    ...
    def evaluate_attenuation(self):
        ...
        integrand = np.gradient( n_e_xg_reshaped  / self.emitter.gamma_e[None, :, None]**2, self.emitter.gamma_e, axis=1)
