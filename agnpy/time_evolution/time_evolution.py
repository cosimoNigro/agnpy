...
class TimeEvolution:
    ...
    def __init__(
        ...
        if isinstance(step_duration, Quantity):
            ...
            raise ValueError(
                'step_duration must be a time Quantity, or a string "auto"'
            )
        self._energy_change_functions = (
            energy_change_functions
            if isinstance(energy_change_functions, dict)
            else (
                {str(v): v for v in energy_change_functions}
                if isinstance(energy_change_functions, Iterable)
                else {str(energy_change_functions): energy_change_functions}
            )
        )
        self._rel_injection_functions = (
            rel_injection_functions
            if isinstance(rel_injection_functions, dict)
            else (
                {str(v): v for v in rel_injection_functions}
                if isinstance(rel_injection_functions, Iterable)
                else (
                    {str(rel_injection_functions): rel_injection_functions}
                    if rel_injection_functions is not None
                    else {}
                )
            )
        )
        self._abs_injection_functions = (
            abs_injection_functions
            if isinstance(abs_injection_functions, dict)
            else (
                {str(v): v for v in abs_injection_functions}
                if isinstance(abs_injection_functions, Iterable)
                else (
                    {str(abs_injection_functions): abs_injection_functions}
                    if abs_injection_functions is not None
                    else {}
                )
            )
        )
        duplicated_keys = (
            set(self._energy_change_functions) & set(self._rel_injection_functions)
        ...
        if duplicated_keys:
            raise ValueError(
                "Found duplicate keys of energy change or injection functions: "
                + str(duplicated_keys)
            )
        ...
        if gamma_bounds is not None and initial_gamma_array is not None:
            if np.any(initial_gamma_array < gamma_bounds[0]):
                raise ValueError(
                    "initial_gamma_array might not contain elements smaller than gamma_min"
                )
            if np.any(initial_gamma_array > gamma_bounds[1]):
                raise ValueError(
                    "initial_gamma_array might not contain elements greater than gamma_max"
                )
        if initial_gamma_array is not None and any(
            initial_gamma_array[1:] <= initial_gamma_array[:-1]
        ):
        ...
        if initial_gamma_array is not None:
            ...
            gamma_min = np.max(
                [
                    self._blob.n_e.gamma_min,
                    gamma_bounds[0] if gamma_bounds is not None else 0,
                ]
            )
            gamma_max = np.min(
                [
                    self._blob.n_e.gamma_max,
                    gamma_bounds[-1] if gamma_bounds is not None else np.inf,
                ]
            )
            self._initial_gamma_array = np.logspace(
                np.log10(gamma_min), np.log10(gamma_max), self._blob.gamma_e_size
            )
        ...
        if optimize_recalculating_slow_rates and step_duration != "auto":
            raise ValueError(
                "optimize_recalculating_slow_rates is only supported with automatic step duration"
            )
        ...
        if optimize_recalculating_slow_rates and method.lower() == "heun":
            raise ValueError(
                "Heun method is not supported when optimize_recalculating_slow_rates is set"
            )
        ...
        for subgroup in subgroups or []:
            for s in subgroup:
                if (
                    s not in self._energy_change_functions
                    and s not in self._rel_injection_functions
                    and s not in self._abs_injection_functions
                ):
        ...
        if subgroups_initial_density is not None and initial_gamma_array is None:
            raise ValueError(
                "subgroups_initial_density provided without initial_gamma_array"
            )
        if (
            subgroups_initial_density is not None
            and initial_gamma_array is not None
            and subgroups_initial_density.shape[1] != len(initial_gamma_array)
        ):
    ...
    def _calculate_initial_values(self):
        ...
        if self._subgroups is not None:
            if self._subgroups_initial_density is not None:
                ...
                subgroups_density = np.zeros(
                    (len(self._subgroups), len(self._initial_gamma_array)), dtype=float
                )
                subgroups_density[0] = np.ones(
                    len(self._initial_gamma_array), dtype=float
                )
        else:
            subgroups_density = np.ones(
                (1, len(self._initial_gamma_array)), dtype=float
            )
        en_chg_rates = recalc_new_rates(
            gm_bins, self._energy_change_functions, density, subgroups_density
        )
        rel_injection_rates = recalc_new_rates(
            gm_bins_lb, self._rel_injection_functions, density, subgroups_density
        )
        abs_injection_rates = recalc_new_rates(
            gm_bins_lb, self._abs_injection_functions, density, subgroups_density
        )
        return (
            gm_bins,
            density,
    ...
    def evaluate(self) -> TimeEvaluationResult:
        ...
        log.info(
            "Progress: %3d%% %s (%i bins)", 0, total_time_elapsed, gm_bins.shape[-1]
        )
        ...
        while total_time_elapsed < self._total_time_sec:
            ...
            (
                en_chg_rates_grouped,
                rel_injection_rates_grouped,
                abs_injection_rates_grouped,
            ) = self._group_change_rates(
            ...
            total_injection_rates_grouped = (
                densities_grouped * rel_injection_rates_grouped
                + abs_injection_rates_grouped
            )
            if self._step_duration != "auto":
                ...
                if self._optimize_recalculating_slow_rates:
                    ...
                    time_left_per_bin[bins_with_no_time_left] = max_times[
                        bins_with_no_time_left
                    ]
            ...
            if self._optimize_recalculating_slow_rates:
                ...
                time_left_per_bin[gm_bins[1] == 0] = (
                    0  # gm_bins[1]==0 is a signal the bin needs recalculation
                )
            ...
            en_chg_rates = recalc_new_rates(
                ...
                density,
                subgroups_density,
                remap(mapping, en_chg_rates, np.nan),
                recalc_mask,
            )
            abs_injection_rates = recalc_new_rates(
                gm_bins[0],
                ...
                density,
                subgroups_density,
                remap(mapping, abs_injection_rates, np.nan),
                recalc_mask,
            )
            rel_injection_rates = recalc_new_rates(
                gm_bins[0],
                ...
                density,
                subgroups_density,
                remap(mapping, rel_injection_rates, np.nan),
                recalc_mask,
            )
            ...
            if self._distribution_change_callback is not None:
                self._distribution_change_callback(
                    TimeEvaluationResult(
                        ...
                        density,
                        subgroups_density,
                        energy_changes_lb(en_chg_rates),
                        abs_injection_rates,
                        rel_injection_rates,
                    )
                )
            progress_percent = int(
                (100 * total_time_elapsed / self._total_time_sec).round(0)
            )
            log.info(
                "Progress: %3d%% %s (%i bins)",
                progress_percent,
                total_time_elapsed,
                gm_bins.shape[-1],
            )
    ...
    def _calc_max_time_per_bin(
        ...
        for i in range(subgroups_density.shape[0]):
            ...
            dummy_denominator = (
                1 * u.erg / u.s
            )  # dummy value, to prevent "division by 0" warnings
            denominator = np.where(
                en_chg_rates != 0, abs(en_chg_rates), dummy_denominator
            )
            time_1 = np.min(
                np.where(
                    en_chg_rates != 0,
                    self._max_energy_change_per_interval * en_bins / denominator,
                    np.inf,
                ),
                axis=0,
            )
            ...
            f2_f1 = (
                en_chg_rates[1] - en_chg_rates[0]
            )  # Δf(x), can be positive/0/negative
            mask = f2_f1 != 0
            ...
            time_3[rel_injection_rates < 0] = (
                -0.5 / rel_injection_rates[rel_injection_rates < 0]
            )
            ...
            time_4[abs_injection_rates != 0] = (
                self._max_injection_per_interval
                / np.abs(abs_injection_rates[abs_injection_rates != 0])
            )
    ...
    def _apply_time_changes(
        ...
        new_gm_bins, new_dens, subgroups_density = self._apply_time_eval(
            en_bins,
            density,
            subgroups_density,
            en_changes_grouped * unit_time_interval_sec,
        ...
        if self._method.lower() == "heun":
            en_chg_rates_recalced = recalc_new_rates(
                new_gm_bins, self._energy_change_functions, new_dens, subgroups_density
            )
            en_changes_grouped_recalced = sum_change_rates(
                en_chg_rates_recalced, new_gm_bins.shape, erg_per_s, self._subgroups
            )
            averaged_en_chg_rates_grouped = (
                en_changes_grouped + en_changes_grouped_recalced
            ) / 2
        ...
        new_gm_bins, new_dens, sort_and_merge_mapping = self._sort_and_merge_bins(
            new_gm_bins, new_dens, dist
        )
        subgroups_density = remap_subgroups_density(
            sort_and_merge_mapping, subgroups_density
        )
        final_mapping = sort_and_merge_mapping

        if self._gamma_bounds is not None:
            # ======== stage 4: remove bins falling behind the edges ========
            new_gm_bins, mapping_wo_bins_beyond_bounds = (
                self._remove_gamma_beyond_bounds(new_gm_bins)
            )
            new_dens = remap(mapping_wo_bins_beyond_bounds, new_dens)
            subgroups_density = remap_subgroups_density(
                mapping_wo_bins_beyond_bounds, subgroups_density
            )
            final_mapping = combine_mapping(
                final_mapping, mapping_wo_bins_beyond_bounds
            )

            # ======== stage 5: add bins at edges if they move towards the center ========
            new_gm_bins, mapping_with_new_bins = self._add_boundary_bins(
                new_gm_bins, en_changes_grouped[0]
            )
            new_dens = remap(mapping_with_new_bins, new_dens, 0 * u.Unit("cm-3"))
            subgroups_density = remap_subgroups_density(
                mapping_with_new_bins, subgroups_density
            )
    ...
    def _apply_time_eval(
        ...
        new_bins_and_densities = [
            recalc_gamma_bins_and_density(
                en_bins, abs_en_changes_grouped[i], density * subgroups_density[i]
            )
        ...
        for i in range(len(new_bins_and_densities)):
            ...
            new_bins_and_densities[i] = BinsWithDensities(
                new_bins_and_densities[i].gamma_bins, new_bin_density
            )
        ...
        for i in range(1, len(new_bins_and_densities)):
            ...
            if np.all(new_dens_subgroup == 0) or np.all(
                new_bins_and_densities[i].gamma_bins == new_gm_bins
            ):
                new_densities[i] = new_dens_subgroup
            else:
                distribution = InterpolatedDistribution(
                    new_gm_bins_subgroup[0],
                    new_dens_subgroup,
    ...
    def _add_boundary_bins(self, gm_bins, en_chg_rates):
        ...
        is_first_bin_far_from_start = (
            gm_bins_log[0] - gamma_bounds_log[0] >= self._max_bin_creep_from_bounds
        )
        if is_first_bin_far_from_start:
            ...
            if first_bin_en_chg > 0:
                ...
                mapping_with_edge_bins = np.concatenate(
                    [np.array([-1]), mapping_with_edge_bins]
                )
        is_last_bin_far_from_end = (
            gamma_bounds_log[-1] - gm_bins_log[-1] >= self._max_bin_creep_from_bounds
        )
        if is_last_bin_far_from_end:
            ...
            if last_bin_en_chg < 0:
                ...
                new_col = np.array(
                    [[self._gamma_bounds[1] / (1 + bin_size_factor)], [0]]
                )
                gm_bins = np.concatenate([gm_bins, new_col], axis=1)
                mapping_with_edge_bins = np.concatenate(
                    [mapping_with_edge_bins, np.array([-1])]
                )
        return gm_bins, mapping_with_edge_bins

    def _group_change_rates(
        self, gm_bins, en_chg_rates, rel_injection_rates, abs_injection_rates
    ):
        en_chg_rates_grouped = sum_change_rates(
            en_chg_rates, gm_bins.shape, erg_per_s, self._subgroups
        )
        rel_injection_rates_grouped = sum_change_rates(
            rel_injection_rates, gm_bins.shape[-1:], per_s, self._subgroups
        )
        abs_injection_rates_grouped = sum_change_rates(
            abs_injection_rates, gm_bins.shape[-1:], per_s_cm3, self._subgroups
        )
        return (
            en_chg_rates_grouped,
            rel_injection_rates_grouped,
    ...
    def _sort_and_merge_bins(self, gm_bins, densities, interpolated_distribution):
        gm_bins, densities, mapping_deduplication = sort_and_merge_duplicates(
            gm_bins, densities, interpolated_distribution
        )
        if self._merge_bins_closer_than is None:
            return gm_bins, densities, mapping_deduplication
        gm_bins, densities, mapping_too_close = self._remove_too_close_bins(
            gm_bins, densities, interpolated_distribution
        )
    ...
    def _remove_gamma_beyond_bounds(self, gm_bins):
        ...
        mapping = np.where(
            (self._gamma_bounds[0] <= gm_bins_lb)
            & (gm_bins_lb <= self._gamma_bounds[1] / (1 + bin_size_factor))
        )[0]
        removed = np.setdiff1d(initial, mapping)
        if len(removed) > 0:
            log.info(
                "Removed bins %s with gamma values %s", removed, gm_bins_lb[removed]
            )
    ...
    def _remove_too_close_bins(self, gm_bins, densities, interpolated_distribution):
        ...
        if np.any(too_close_bins_mask):
            ...
            keep_idx = np.setdiff1d(
                np.arange(len(densities)), close_bins_idx, assume_unique=True
            )
            close_bins_groups = group_duplicates(close_bins_idx)
            log.info("Merging groups of bins %s", close_bins_groups)
            gm_bins_lb_merged, densities_merged = merge_points(
                gm_bins[0], densities, close_bins_groups, interpolated_distribution
            )
            ...
            return (
                np.array([gm_bins_lb_merged, gm_bins_ub_merged]),
                densities_merged,
                keep_idx,
            )
        else:
