...
class LIVdelay:
    ...
    def __init__(self, lambda_liv, n_liv, blob, nu_syn, synch, time_step, steps):
        ...
        self.n_liv = n_liv
        self.time_evoluted_seds = np.array() # so a 2D matrix, containing the spectral evolution as a function of time
        self.blob = blob
        self.synch = synch
    ...
    def calculate_time_evoluted_seds(self):
        ...
        for i in range(self.steps):
            ...
            time_evoluted_seds.append(synch.sed_flux(self.nu_syn))
        self.time_evoluted_seds = np.array(time_evoluted_seds)
        return self.time_evoluted_seds
    ...
    def evaluate(self, time_obs):
        ...
        """
        calculate_time_evoluted_seds()
        time_evoluted_seds_liv = np.zeros(len(self.nu_syn))
        for nu_index, nu in enumerate(nu_syn):
            ...
            time_evoluted_seds_liv[nu_index] = interpolate_seds(self.steps, self.time_step, nu_index, time_obs_LIV, self.time_evoluted_seds)

        return time_evoluted_seds_liv
