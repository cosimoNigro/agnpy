...
class DistributionToSinglePointCollapseError(Exception):
    def __init__(self, gamma_point):
        ...
        super().__init__(
            f"Unsupported state, cannot create InterpolatedDistribution - distribution collapsed to a single gamma point {gamma_point}"
        )
...
def recalc_new_rates(
    gm_bins: NDArray,
    functions: dict[str, GammaFn],
    dens: Quantity,
    subgroups_density: NDArray,
    previous_rates=None,
    recalc_mask=None,
) -> dict[str, Quantity]:
    """
    ...
    if previous_rates is None:
        previous_rates = {}
    mask = (
        np.ones((gm_bins.shape[-1]), dtype=bool) if recalc_mask is None else recalc_mask
    )
    gm_bins_masked = gm_bins[..., mask]
    gm_bins_interlaced = gm_bins_masked.reshape(
        -1, order="F"
    )  # order F gives more natural, increasing order (makes debugging easier)
    dens_masked = dens[mask]
    dens_groups_masked = remap_subgroups_density(
        mask_to_mapping(mask), subgroups_density
    )
    ...
    for label, fn in functions.items():
        if recalc_mask is None:
            ...
            new_rates[label][..., mask] = fn(params).reshape(
                gm_bins_masked.shape, order="F"
            )
    return new_rates


def sum_change_rates(
    change_rates: dict[str, Quantity],
    shape: Tuple,
    unit: Unit,
    ...
    if subgroups is None:
        ...
        return Quantity(
            [
                (
                    np.sum(
                        Quantity([change_rates[x] for x in row if x in change_rates]),
                        axis=0,
                    )
                    if any(x in change_rates for x in row)
                    else np.zeros(shape) * unit
                )
...
def to_densities_grouped(density, subgroups_density):
    density_grouped = (
        ...
        if subgroups_density is not None
        else density.reshape((1, len(density)))
    )
...
def recalc_gamma_bins_and_density(
    energy_bins, abs_energy_changes, density
) -> BinsWithDensities:
    new_energy_bins, density_increase = recalc_energy_bins_and_density_increase(
        abs_energy_changes, energy_bins
    )
...
def recalc_energy_bins_and_density_increase(abs_energy_changes, energy_bins):
    ...
    if np.any(invalid_bins):
        raise ValueError(
            "Negative gamma obtained at bins "
            + str(np.arange(len(energy_bins_lb))[invalid_bins])
        )
    ...
    if np.any(invalid_width):
        raise ValueError(
            "Negative density obtained at bins "
            + str(np.arange(len(energy_bins_lb))[invalid_width])
        )
...
def sort_and_merge_duplicates(gm_bins, densities, interpolated_distribution):
    ...
    if any(duplicate_idx):
        keep_idx = np.setdiff1d(
            np.arange(len(sort_idx)), duplicate_idx, assume_unique=True
        )
        ...
        return (
            np.array([gm_bins_lb_merged, np.zeros_like(gm_bins_lb_merged)]),
            densities_merged,
            sort_idx[keep_idx],
        )
    else:
...
def merge_points(x, y, merge_groups, interpolated_distribution):
    ...
    for g_idx, group in enumerate(merge_groups):
        labels[group] = g_idx
    x_means = np.array(
        [np.exp(np.mean(np.log(x[labels == g]))) for g in range(len(merge_groups))]
    )
    if interpolated_distribution is None:
        y_means = Quantity([np.mean(y[labels == g]) for g in range(len(merge_groups))])
    else:
        y_means = Quantity(
            [interpolated_distribution(x_means[g]) for g in range(len(merge_groups))]
        )
    ...
    merged_x = np.where(
        labels_destination == -1, x[retain_mask], x_means[labels_destination]
    )
    merged_y = np.where(
        labels_destination == -1, y[retain_mask], y_means[labels_destination]
    )
...
def update_distribution(gamma_array, n_array, blob):
    gamma_sorted_and_merged, n_array_sorted_and_merged, _ = sort_and_merge_duplicates(
        gamma_array, n_array, None
    )
    if np.all(n_array_sorted_and_merged == 0):
        distribution = EmptyDistribution(
            gamma_min=gamma_sorted_and_merged[0], gamma_max=gamma_sorted_and_merged[-1]
        )
    else:
        ...
        distribution = InterpolatedDistribution(
            gamma_sorted_and_merged[0],
            n_array_sorted_and_merged,
