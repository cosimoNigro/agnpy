...
from agnpy.time_evolution._time_evolution_utils import (
    to_total_energy_gev_sqr,
    DistributionToSinglePointCollapseError,
)
from agnpy.time_evolution.time_evolution import (
    TimeEvolution,
    synchrotron_loss,
    ssc_loss,
    ssc_thomson_limit_loss,
    fermi_acceleration,
)
...
class TestSpectraTimeEvolution:
    ...
    def test_compare_numerical_results_with_analytical_calculation_synch(
        self, p, time_and_steps
    ):
        """
        ...
        def gamma_before_synch(gamma_after_time, time):
            return gamma_before_time(
                synch._electron_energy_loss_formula_prefactor, gamma_after_time, time
            )

        assert u.isclose(
            gamma_before_synch(evaluated_n_e.gamma_min, time), gamma_min, rtol=0.05
        )
        assert u.isclose(
            gamma_before_synch(evaluated_n_e.gamma_max, time), gamma_max, rtol=0.05
        )
        assert u.isclose(
        ...
        assert u.isclose(
            ...
            evaluated_n_e.integrate(
                evaluated_n_e.gamma_max / 10, evaluated_n_e.gamma_max
            ),
            power_law_integral(
                k, p, gamma_before_synch(evaluated_n_e.gamma_max / 10, time), gamma_max
            ),
            rtol=0.05,
        )
    ...
    def test_compare_calculation_with_calculation_split_into_two(self):
        ...
        TimeEvolution(
            blob, 60 * u.s, synchrotron_loss(synch), step_duration=3 * u.s
        ).evaluate()
        ...
        TimeEvolution(
            blob, 30 * u.s, synchrotron_loss(synch), step_duration=3 * u.s
        ).evaluate()
        TimeEvolution(
            blob, 30 * u.s, synchrotron_loss(synch), step_duration=3.5 * u.s
        ).evaluate()
        eval_2 = blob.n_e
    ...
    def test_total_number_of_electrons_should_stay_after_time_evolution(self):
        ...
        TimeEvolution(
            ...
            max_density_change_per_interval=10.0,
        ).evaluate()
    ...
    def test_ssc_losses_in_thompson_limit(self):
        ...
        thomson = ssc.electron_energy_loss_rate_thomson(np.logspace(1, 3, 200)).to(
            "erg/s"
        )
    ...
    def test_compare_numerical_results_with_analytical_calculation_ssc_thomson(self):
        ...
        def gamma_before_ssc_th(gamma_after_time, before_time):
            return gamma_before_time(
                ssc._electron_energy_loss_thomson_formula_prefactor,
                gamma_after_time,
                before_time,
            )

        # map result gammas to original gammas and assert they are equal
        for evaluated_gamma, initial_gamma in zip(
            evaluated_n_e.gamma_input, initial_gammas
        ):
            assert u.isclose(
                gamma_before_ssc_th(evaluated_gamma, time), initial_gamma, rtol=0.001
            )

        # split the result into log-equal intervals, integrate them, and compare with the analytical result
        logspace = np.logspace(
            np.log10(n_e.gamma_min), np.log10(evaluated_n_e.gamma_max), 20
        )
        for gamma_min, gamma_max in zip(logspace, logspace[1:]):
            ...
            assert u.isclose(
                ...
                power_law_integral(
                    k,
                    p,
                    gamma_before_ssc_th(gamma_min, time),
                    gamma_before_ssc_th(gamma_max, time),
                ),
                rtol=0.001,
            )
    ...
    def test_combined_energy_loss_from_two_processes(self):
        ...
        initial_gammas = blob.gamma_e
        decrease_by_20_perc = (
            lambda args: -1 * (args.gamma * mec2).to("erg") * 0.2 / u.s
        )
        ...
        TimeEvolution(
            blob,
            number_of_steps * u.s,
            [decrease_by_20_perc, increase_by_5_perc],
            method="euler",
            max_energy_change_per_interval=0.2,
            max_density_change_per_interval=0.5,
            step_duration=1 * u.s,
        ).evaluate()
    ...
    def test_heun_method_compared_to_euler(self):
        ...
        TimeEvolution(
            blob1,
            time,
            synchrotron_loss(synch1),
            method="euler",
            step_duration=0.1 * u.s,
        ).evaluate()
        euler_reversed_analytically = gamma_before_time(
            synch1._electron_energy_loss_formula_prefactor, blob1.gamma_e, time
        )
        ...
        TimeEvolution(
            blob2,
            time,
            synchrotron_loss(synch2),
            method="heun",
            step_duration=0.2 * u.s,
        ).evaluate()
        heun_reversed_analytically = gamma_before_time(
            synch2._electron_energy_loss_formula_prefactor, blob2.gamma_e, time
        )

        errors_heun = np.abs(
            (heun_reversed_analytically - initial_gamma) / initial_gamma
        )
        errors_euler = np.abs(
            (euler_reversed_analytically - initial_gamma) / initial_gamma
        )
    ...
    def test_heun_method_compared_to_euler_for_automatic_intervals_method(self):
        ...
        TimeEvolution(
            ...
            method="euler",
            optimize_recalculating_slow_rates=False,
        ).evaluate()
        euler_reversed_analytically = gamma_before_time(
            synch1._electron_energy_loss_formula_prefactor, blob1.gamma_e, time
        )
    ...
    def test_automatic_and_fixed_intervals_approach_with_single_loop(self):
        ...
        synch1 = Synchrotron(blob1)
        TimeEvolution(
            blob1, time, synchrotron_loss(synch1), step_duration=time
        ).evaluate()

        blob2 = Blob(n_e=PowerLaw())
        synch2 = Synchrotron(blob2)
        TimeEvolution(
            blob2, time, synchrotron_loss(synch2), max_energy_change_per_interval=0.5
        ).evaluate()

        assert np.all(
            np.isclose(blob1.n_e.gamma_input, blob2.n_e.gamma_input, rtol=0.000001)
        )
    ...
    def test_simulation_trending_to_delta_function(self):
        ...
        fermi_acc = lambda args: (blob.xi * blob.B_cgs * c * e.gauss) * np.ones_like(
            args.gamma
        )
        ...
        with pytest.raises(DistributionToSinglePointCollapseError) as ex:
            TimeEvolution(
                blob,
                time,
                [synchrotron_loss(synch), fermi_acc],
                max_energy_change_per_interval=0.1,
    ...
    def test_synch_losses_with_continues_injection_and_acceleration(self):
        ...
        blob = Blob(
            ...
            Gamma,
            B,
            n_e=EmptyDistribution(gamma_min=10, gamma_max=1e3),
        )
        synch = Synchrotron(blob)
        injection_dist = PowerLaw.from_total_energy(
            100000 * u.GeV, V_b, gamma_min=10, gamma_max=1000, p=2, mass=m_e
        )
        ...
        for i in range(len(times) - 1):
            ...
            _, gamma_array, densities, _, energy_change_rates, _, _ = TimeEvolution(
                ...
                energy_change_functions={
                    "Synch": synchrotron_loss(synch),
                    "Acc": fermi_acceleration(tacc),
                },
                abs_injection_functions={"Injection": injection_function},
            ...
            if i < 4:
                sed_nu_fnu = synch.sed_flux(
                    sed_x * ev_per_gamma.to("Hz", equivalencies=u.spectral())
                )
            else:
            ...
            df = pd.DataFrame({"eed_y": eed_y.value, "sed_y": sed_y.value})
            ...
            ignore_pos = [
                31,
                40,
            ]  # positions where agnpy returns already -inf but reference data still haves some very small value
            assert_series_equal_ignore_pos(
                ref.sed_y.apply(np.log10),
                df.sed_y.apply(np.log10),
                ignore_pos,
                rtol=0.5,
                atol=0.6,
            )

    def test_synch_losses_with_continues_injection_and_acceleration_with_two_zones(
        self,
    ):
        ...
        synch = Synchrotron(blob)
        injection_dist = PowerLaw.from_total_energy(
            100000 * u.GeV, V_b, gamma_min=10, gamma_max=20, p=2, mass=m_e
        )
        ...
        number_of_bins = 11
        density_groups = np.array(
            [
                np.ones(number_of_bins, dtype=float),
                np.zeros(number_of_bins, dtype=float),
            ]
        )
        gamma_array = np.logspace(np.log10(10), np.log10(20), number_of_bins)
        for i in range(len(times) - 1):
            single_iteration_time = (times[i + 1] - times[i]) * u.s
            _, gamma_array, densities, density_groups, energy_change_rates, _, _ = (
                TimeEvolution(
                    blob,
                    single_iteration_time,
                    energy_change_functions={
                    ...
                    abs_injection_functions={
                        "Zone1-inj": injection_function_zone1,
                        "Zone2-inj": injection_function_zone2,
                    },
                    rel_injection_functions={"Zone1-esc": escape_fn},
                    subgroups=[
                        ["Synch", "Acc", "Zone1-inj", "Zone1-esc"],
                        ["Synch", "Zone2-inj"],
                    ],
            ...
            distribution_zone1 = InterpolatedDistribution(
                gamma_array, densities * density_groups[0]
            )
            distribution_zone2 = InterpolatedDistribution(
                gamma_array, densities * density_groups[1]
            )
            ref = ref_data[i]
            eed_x = ref.eed_x.values
            ...
            ignore_pos = [
                ...
                13,
                29,
            ]  # positions where sharp density changes occur at different stages; for simplicity, just filter them all out
            assert_series_equal_ignore_pos(
                ref.eed_y_zone1, df.eed_y_zone1, ignore_pos, rtol=0.3
            )
            assert_series_equal_ignore_pos(
                ref.eed_y_zone2, df.eed_y_zone2, ignore_pos, rtol=0.3
            )
    ...
    def test_ssc_losses_compared_with_different_calculation_approach(self):
        ...
        n_e_pl = PowerLaw.from_total_energy(
            0.3e45 * u.erg, V_b, p=2, gamma_min=1e4, gamma_max=1e7, mass=m_e
        )
        ...
        for i in range(1, 4):
            TimeEvolution(
                blob,
                30 * u.s,
                energy_change_functions=[synchrotron_loss(synch), ssc_loss(ssc)],
            ).evaluate()
            ref = ref_data[i]
            df = pd.DataFrame(
                {
            ...
            if i == 1:
                assert_series_equal_ignore_pos(
                    ref.eed_y, df.eed_y, ignore_pos=[4, 15], rtol=0.1
                )
                assert_series_equal_ignore_pos(
                    ref.eed_y, df.eed_y, ignore_pos=[], rtol=0.35
                )
            elif i == 2:
                assert_series_equal_ignore_pos(
                    ref.eed_y, df.eed_y, ignore_pos=[12], rtol=0.1
                )
                assert_series_equal_ignore_pos(
                    ref.eed_y, df.eed_y, ignore_pos=[], rtol=0.35
                )
                ...
                assert_series_equal_ignore_pos(
                    ref.eed_y, df.eed_y, ignore_pos=[3], rtol=0.1
                )
    ...
    def test_total_electron_energy_loss_is_equal_to_total_radiation_energy(self):
        ...
        n_e_pl = PowerLaw.from_total_energy(
            1e70 * u.erg, V, p=2, gamma_min=gamma_min, gamma_max=gamma_max, mass=m_e
        )
        ...
        synchrotron_losses = np.trapezoid(
            -1 * synchrotron_loss(synchrotron)(params) * density, gamma_logspace
        )
        ssc_losses = np.trapezoid(-1 * ssc_loss(ssc)(params) * density, gamma_logspace)

        sphere_area = Distance(z=blob.z).to("cm") ** 2 * 4 * np.pi
        synchrotron_radiation_total = (
            synchrotron.energy_flux_integral(1e5 * u.Hz, 1e30 * u.Hz) * sphere_area
        )
        ssc_radiation_total = (
            ssc.energy_flux_integral(1e5 * u.Hz, 1e30 * u.Hz) * sphere_area
        )
