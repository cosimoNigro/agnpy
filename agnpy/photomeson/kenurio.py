...
def H_integrand(gamma, eta, gamma_limit, particle_dist, soft_photon_dist, particle, *args):

    return (1 / gamma ** 2  *
        particle_dist(gamma, *args).value *
...
class PhotomesonGamma:
    ...
    def __init__(self,particle_dist, soft_photon_distribution, integrator = np.trapz):
    ...
    @staticmethod
    def spectrum(epsilons, particle_distribution, soft_photon_distribution, particle):

        spectrum_array = np.zeros(len(epsilons))
        for i, epsilon in enumerate(epsilon):

            gamma_range = [epsilon, 1e13] # do not change upper limit: gamma_max = 1e13
            eta_range = [0.313, 31.3]
            ...
            print("Executing {} out of {} steps...\n dNdE={}"
                        .format(i + 1, len(epsilons), spectrum_array[i]))
    ...
    def evaluate_sed_flux(
        ...
        sed_source_frame = (
                PhotomesonElectron.spectrum(
                epsilon_prime, n_p, soft_photon_distribution, 'electron') * (vol / area) * (epsilon_prime * mpc2.to('erg')) ** 2
        ).to("erg cm-2 s-1")
    ...
    def sed_flux(self, gamma, particle):
...
class PhotomesonElectron:
    ...
    @staticmethod
    def spectrum(gammas, n_p, soft_photon_distribution, particle):

        spectrum_array = np.zeros(len(gammas))

        for i, gamma in enumerate(gammas):

            gamma_range = [gamma*mec2/mpc2 , 1e13 ]
            eta_range = [0.945, 31.3]

            dNdE = (1 / 4) * (mpc2.value) *  nquad(H_integrand,
                                        [gamma_range, eta_range],
                                        args=[gamma * mec2 / mpc2,
                                        n_p,
                                        soft_photon_distribution,
                                        particle]
                                        )[0]
    ...
    def evaluate_sed_flux(
        ...
        sed_source_frame = (
                PhotomesonElectron.spectrum(
                gamma_prime, n_p, soft_photon_distribution, 'electron') * (vol / area) * (gamma_prime * mec2.to('erg')) ** 2
        ).to("erg cm-2 s-1")
...
if __name__ == '__main__':
    ...
    proton_gamma = PhotomesonElectron(blob, g_dist)
    sed2 = proton_gamma.sed_flux(gammas_prime, 'electron')
