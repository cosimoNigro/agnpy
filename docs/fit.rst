...
The wrappers consider the combination of several radiative processes and return a ``model`` object that can be used by the fitting routine of the two packages.
...
Prerequisites
-------------
Note that the :py:mod:`agnpy.fit` module, containing the wrappers, needs ``gammapy`` and ``sherpa`` to be installed.
...
The following warning message will be printed by ``agnpy`` if the packages have not been installed:

``WARNING:root:sherpa and gammapy are not installed, the agnpy.fit module cannot be used``

In this case, all ``agnpy`` modules except :py:mod:`agnpy.fit`, can be used normally.
...
----------------------------------------------------
We provide, in ``agnpy.fit``, functions that can directly load flux points in ``sherpa`` and ``Gammapy`` data objects.
The SED format that these functions can read follows the one proposed by the `Data Formats for Gamma-ray Astronomy <https://gamma-astro-data-formats.readthedocs.io/en/latest/spectra/flux_points/index.html>`_.
